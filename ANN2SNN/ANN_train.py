...
# Training
flags.DEFINE_string('dataset_type', 'cifar10', help='dataset type')
flags.DEFINE_float('lr', 2e-4, help='target learning rate')
flags.DEFINE_float('grad_clip', 1., help="gradient norm clipping")
flags.DEFINE_integer('total_steps', 800000, help='total training steps')#800000
    ...
    with trange(FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            torch.nn.utils.clip_grad_norm_(
                net_model.parameters(), FLAGS.grad_clip)#梯度裁剪
            optim.step()
            sched.step()
            ...
            if FLAGS.eval_step > 0 and step % FLAGS.eval_step == 0:
                ...
                writer.flush()
                with open(os.path.join(FLAGS.logdir, 'snn_eval.txt'), 'a') as f:
                    metrics['step'] = step
                    f.write(json.dumps(metrics) + "\n")
            '''
            # para check
            if FLAGS.sample_step > 0 and step % FLAGS.sample_step == 0 and False:
                net_model.show_params()
            '''
    writer.close()
